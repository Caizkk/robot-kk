...
function requireNodeExternalApi() {
  ...
  class NodeExternalApi {
    ...
    getSystemPathHome() {
      var _a;
      return ((_a = os.homedir) == null ? void 0 : _a.call(os)) || {}.HOME;
    }
    getSystemPathTemp() {
      return os.tmpdir();
    }
...
require$$0$5.contextBridge.exposeInMainWorld("electronAPI", {
  on(...args) {
    const [channel, listener] = args;
    return require$$0$5.ipcRenderer.on(channel, (event, ...args2) => listener(event, ...args2));
  },
  off(...args) {
    const [channel, ...omit] = args;
    return require$$0$5.ipcRenderer.off(channel, ...omit);
  },
  send(...args) {
    const [channel, ...omit] = args;
    return require$$0$5.ipcRenderer.send(channel, ...omit);
  },
  invoke(...args) {
    const [channel, ...omit] = args;
    return require$$0$5.ipcRenderer.invoke(channel, ...omit);
  },
  // 应用信息API
  getAppInfo: () => {
    return {
  ...
  // 系统信息API
  getSystemInfo: () => {
    return require$$0$5.ipcRenderer.invoke("get-system-info");
  },
  // 日志文件操作API
  openLogFile: (logPath = DEFAULT_LOG_PATH) => {
    return require$$0$5.ipcRenderer.invoke("open-log-file", logPath);
  },
  getDefaultLogPath: () => {
  ...
  // 窗口控制API
  windowControl: {
    minimize: () => require$$0$5.ipcRenderer.send("window-minimize"),
    close: () => require$$0$5.ipcRenderer.send("window-close"),
    drag: () => require$$0$5.ipcRenderer.send("window-drag"),
    show: () => require$$0$5.ipcRenderer.send("window-show")
  },
  // 屏幕信息相关API
  screen: {
    // 获取当前缩放因子
    getScaleFactor: () => require$$0$5.ipcRenderer.invoke("get-scale-factor"),
    // 获取屏幕信息
    getInfo: () => require$$0$5.ipcRenderer.invoke("get-screen-info"),
    // 重新加载窗口
    reloadWindow: () => require$$0$5.ipcRenderer.send("reload-window"),
    // 监听屏幕变化事件
    onChanged: (callback) => {
      require$$0$5.ipcRenderer.on("screen-changed", (_, screenInfo) => {
        callback(screenInfo);
      });
    },
    // 移除屏幕变化监听
    offChanged: () => {
      require$$0$5.ipcRenderer.removeAllListeners("screen-changed");
    },
    // 获取缩放因子历史
    getScaleFactorHistory: () => require$$0$5.ipcRenderer.invoke("get-scale-factor-history"),
    // 设置页面缩放因子
    setZoomFactor: (zoomFactor) => require$$0$5.ipcRenderer.invoke("set-zoom-factor", zoomFactor),
    // 计算最佳缩放因子
    calculateOptimalZoom: (scaleFactor) => {
      if (scaleFactor <= 1)
        return 1;
      if (scaleFactor <= 1.5)
        return 1 - (scaleFactor - 1) * 0.3;
      if (scaleFactor <= 2)
        return 0.85 - (scaleFactor - 1.5) * 0.2;
      return 0.75;
    }
  },
  // Python进程管理API
  pythonProcess: {
    // 获取Python进程状态
    getStatus: () => require$$0$5.ipcRenderer.invoke("get-python-status"),
    // 重启Python进程
    restart: () => require$$0$5.ipcRenderer.invoke("restart-python-process"),
    // 监听Python进程失败事件
    onFailed: (callback) => {
      require$$0$5.ipcRenderer.on("python-process-failed", () => {
        callback();
      });
    },
    // 移除Python进程失败事件监听
    offFailed: () => {
      require$$0$5.ipcRenderer.removeAllListeners("python-process-failed");
    }
  },
  // 日志API
  logger: {
    // 获取日志文件路径
    getLogPath: () => require$$0$5.ipcRenderer.invoke("get-log-path"),
    // 日志级别方法
    ...
    // 打开日志文件目录
    openLogFile: () => {
      require$$0$5.ipcRenderer.invoke("get-log-path").then((paths) => {
        require$$0$5.ipcRenderer.invoke("open-log-file", paths.logFilePath);
      });
    },
    // 打开日志目录
    openLogDirectory: () => {
      require$$0$5.ipcRenderer.invoke("get-log-path").then((paths) => {
        require$$0$5.ipcRenderer.invoke("open-log-file", paths.logDirectory);
      });
    }
  }
});
